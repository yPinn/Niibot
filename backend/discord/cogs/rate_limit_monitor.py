...
class RateLimitMonitorCog(commands.Cog):
    ...
    async def rate_stats(self, interaction: discord.Interaction):
        ...
        embed.add_field(name="最近1分鐘請求數", value=f"`{stats['recent_1min_requests']}`", inline=True)
        embed.add_field(
            name="最近1分鐘平均速率",
    ...
    async def rate_check(self, interaction: discord.Interaction):
        ...
        embed.add_field(name="全局限制", value=f"{self.rate_limiter.GLOBAL_RATE_LIMIT} req/s", inline=True)
        embed.add_field(name="訊息限制", value=f"{self.rate_limiter.MESSAGE_RATE_LIMIT} msg/5s/ch", inline=True)
