...
_SPEC = importlib.util.spec_from_file_location(
    "root_env_init", _ROOT / "scripts" / "env" / "init.py"
)
...
def test_sets_runtime_environment_without_touching_other_values(tmp_path: Path) -> None:
    env_file = tmp_path / "shared.env"
    env_file.write_text("ENVIRONMENT=development\nTOKEN=secret\n", encoding="utf-8")

    changed = ensure_local_env.set_environment(env_file, "staging")

    assert changed is True
    assert env_file.read_text(encoding="utf-8") == "ENVIRONMENT=staging\nTOKEN=secret\n"


def test_set_environment_ignores_files_without_the_assignment(tmp_path: Path) -> None:
    env_file = tmp_path / "api.env"
    env_file.write_text("API_URL=http://localhost\n", encoding="utf-8")

    assert ensure_local_env.set_environment(env_file, "production") is False
    assert env_file.read_text(encoding="utf-8") == "API_URL=http://localhost\n"
