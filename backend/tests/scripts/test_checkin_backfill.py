...
import pytest
from scripts.checkin import backfill as cli

from shared.collection_backfill import BackfillReport


def test_parser_defaults_to_a_report_only_development_preview() -> None:
...
def test_parser_supports_explicit_staging_apply_and_dry_run() -> None:
    apply_args = cli.build_parser().parse_args(["--env", "stg", "--batch-size", "7", "--apply"])
    dry_args = cli.build_parser().parse_args(["--env", "stg", "--dry-run"])

    assert apply_args.env == "stg"
...
@pytest.mark.asyncio
@pytest.mark.parametrize("env_args", [[], ["--env", "stg"]])
...
async def test_apply_returns_failure_when_rows_remain_or_a_viewer_transaction_failed(
    ...
    args = cli.build_parser().parse_args(["--env", "stg", "--apply", "--yes"])
