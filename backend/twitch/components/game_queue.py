...
class GameQueueComponent(BotComponent):
    ...
    async def gq(self, ctx: commands.Context["Bot"]) -> None:
        ...
        if total == 0:
            await self._ctx_reply(ctx, f"[{status}] 無人排隊 | !gq me 查詢自己")
            return
        ...
        await self._ctx_reply(
            ctx, f"[{status}] 下一場: {next_names} | 共{total}人排隊 | !gq me 查詢自己"
        )
    ...
    async def gq_me(self, ctx: commands.Context["Bot"]) -> None:
        ...
        if user_index is None:
            await self._ctx_reply(ctx, "未排隊 | !gq 查看隊列")
            return
    ...
    async def gq_next(self, ctx: commands.Context["Bot"]) -> None:
        ...
        if not to_complete:
            await self._ctx_reply(ctx, "隊列為空")
            return
    ...
    async def gq_kick(self, ctx: commands.Context["Bot"], *, args: str | None = None) -> None:
        ...
        if not args or not args.strip():
            await self._ctx_reply(ctx, "!gq kick @username")
            return
        ...
        if not target:
            await self._ctx_reply(ctx, f"找不到 {target_name}")
            return

        await self.gq_repo.remove_entry(target.id, channel_id, "kicked")
        await self._ctx_reply(ctx, f"已移除 {target.user_name}")
    ...
    async def gq_clear(self, ctx: commands.Context["Bot"]) -> None:
        ...
        await self._ctx_reply(ctx, f"已清空（{cleared} 人）")
    ...
    async def gq_open(self, ctx: commands.Context["Bot"]) -> None:
        ...
        await self._ctx_reply(ctx, "隊列已開啟")
    ...
    async def gq_close(self, ctx: commands.Context["Bot"]) -> None:
        ...
        await self._ctx_reply(ctx, "隊列已關閉")
    ...
    async def gq_size(self, ctx: commands.Context["Bot"], *, args: str | None = None) -> None:
        ...
        await self._ctx_reply(ctx, f"已調整為 {size}人/場")
