...
"""Twitch token / emote diagnostics.
    ...
    npm run nb -- twitch {tokens|emotes} [--env stg]
    uv run --directory backend python -m scripts.twitch_ops.diag {tokens|emotes} [--env stg]
...
def _load_config(env: str) -> None:
    global CLIENT_ID, CLIENT_SECRET, BOT_ID, _BOT_SCOPES_SET, _BROADCASTER_SCOPES_SET, _TOKEN_KEY
    ...
    _TOKEN_KEY = os.getenv("TWITCH_TOKEN_ENCRYPTION_KEY", "")
...
async def _tokens() -> None:
    from shared.twitch_token_crypto import TwitchTokenEncryptionError, decrypt_twitch_token

    conn = await asyncpg.connect(database_url())
    rows = await conn.fetch(
        "SELECT user_id, token, encryption_version FROM tokens ORDER BY user_id"
    )
    ...
    async with httpx.AsyncClient() as client:
        for row in rows:
            uid = row["user_id"]
            try:
                token = decrypt_twitch_token(
                    row["token"], version=int(row["encryption_version"]), key=_TOKEN_KEY
                )
                r = await client.get(
                ...
                tokens_data.append({"uid": uid, "status": "decrypt_error"})
    ...
    for data in tokens_data:
        ...
        elif data["status"] == "decrypt_error":
            print(f"{uid_to_name.get(data['uid'], '?')} ({data['uid']}) - DECRYPT ERROR\n")
        else:
