...
前置: Twitch dev console 的 OAuth Redirect URLs 需含 http://localhost:3010/callback

用法（僅限 dev；stg/prod 走後台授權流程）:
    npm run nb -- twitch oauth [--env dev] [--role bot|broadcaster]
    uv run --directory backend python -m scripts.twitch_ops.oauth [--env dev] [--role ...]
...
from scripts._lib import load_env, require_dev_database, utf8_stdio
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--env", choices=ENVS, help="dev (interactive if omitted)")
    parser.add_argument(
...
def _resolve_env_role(args: argparse.Namespace) -> tuple[str, str]:
    ...
    if env is None:
        env = _pick(
            "選擇環境",
            {env: f"shared.{env}.env + twitch/.env.{env}" for env in ENVS},
        )
    if role is None:
        role = _pick(
            "選擇角色",
...
def run(args: argparse.Namespace) -> int:
    ...
    if env != "dev":
        fail("stg/prod 授權請使用 admin web flow；本工具只供本機 dev。")
    ...
    require_dev_database(database_url or "")

    label, scopes = ROLES[role]
    env_tag = f" {yellow(f'[{env.upper()}]')}" if env != "dev" else ""
    total_steps = 4
