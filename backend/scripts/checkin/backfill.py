...
Examples (report-only is the default):
    ...
    uv run --directory backend python -m scripts.checkin.backfill
    uv run --directory backend python -m scripts.checkin.backfill --env stg --apply
    uv run --directory backend python -m scripts.checkin.backfill --apply --yes
...
from scripts._lib import add_env_arg, confirm, db_conn, ensure_backend_on_path, utf8_stdio
