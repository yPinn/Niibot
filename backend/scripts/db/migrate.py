"""Run database migrations using shared.migrations.runner.
    ...
    npm run nb -- db migrate [--dry] [--env stg]
    uv run --directory backend python -m scripts.db.migrate [--dry] [--env stg]
...
from scripts._lib import add_env_arg, db_pool, ensure_backend_on_path
...
_VERSIONS_DIR = Path(__file__).resolve().parents[2] / "shared" / "migrations" / "versions"
