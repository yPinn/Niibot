...
| 未識別路徑                         |     ✓     |    ✓     |    ✓    |    —     |      —       |
| `main`／`staging` push             |     ✓     |    ✓     |    ✓    |    ✓     | staging only |
| manual／reusable invocation        |     ✓     |    ✓     |    ✓    |    ✓     |      —       |

PR 仍跑完整受影響套件的測試，只把 coverage 留給 protected-branch／manual run。這避免 coverage
instrumentation 在每個小 commit 重複執行，又保留 staging deploy 前的 80% coverage gate。
...
- `main`、`staging`、manual 與 reusable invocation 不看路徑，一律完整執行。
...
- Protected-branch push 以唯一 run id 分組，不互相取消。
...
1. 先要求穩定存在的 Preflight、Frontend、Backend job 名稱；job-level skipped 視為成功。
...
7. PR 跑受影響套件測試；protected branch 跑完整 integration + coverage。
...
- staging deploy 仍只依賴完整成功的 protected-branch CI，且 reusable deploy 的不可取消設定未變。
