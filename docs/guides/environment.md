...
| `frontend/.env.example` | `.env` | Cloudflare 設定 | Cloudflare 設定 |

既有未帶環境的檔案可用 `npm run nb -- env migrate <target>` 搬移；target 必填，避免把
舊檔誤判為 dev 或 prod。搬移或手動調整後執行 `npm run nb -- env validate <target>`，
只檢查 key 集合與順序，不輸出值。若要按新 example 排列並把舊 service-level shared key
搬回 shared 檔，先做 snapshot，再執行 `npm run nb -- env sync <target>`；未知 key 或不同檔案的
非空值衝突會在寫入前中止，錯誤訊息不含值。
...
| `ENVIRONMENT`                 | development / staging / production（api/twitch/discord 共用）                                                     |
...
本機檔案的 key 與順序，再進行任何 GitHub mutation。部署時 `_deploy.yml` 依
`env.manifest.json` 寫出 `.env.<env>` 與各服務 env。詳見 [deployment.md](deployment.md)。
