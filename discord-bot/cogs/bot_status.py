...
class BotStatus(commands.Cog):
    ...
    async def set_bot_status(self):
        ...
        try:
            ...
            status_obj = getattr(discord.Status, config.get_discord_config('STATUS'), discord.Status.online)
            ...
            BotLogger.system_event(
                "狀態設定", 
                f"✅ 狀態: {config.get_discord_config('STATUS')}, 活動: {config.get_discord_config('ACTIVITY_NAME')}"
            )
    ...
    async def status_monitor(self):
        ...
        try:
            ...
            expected_status = getattr(discord.Status, config.get_discord_config('STATUS'), discord.Status.online)
            ...
            if status:
                ...
                if hasattr(discord.Status, status):
                    ...
                    embed = BotStatusEmbeds.create_status_success(
                        status, activity or config.get_discord_config('ACTIVITY_NAME'))
                    await ctx.send(embed=embed)
                    
                    BotLogger.command_used("status", ctx.author.id, ctx.guild.id if ctx.guild else 0, 
                                         f"手動設定狀態: {status}, 活動: {activity or config.get_discord_config('ACTIVITY_NAME')}")
                else:
