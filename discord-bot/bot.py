...
# 初始化日誌系統
BotLogger.initialize(config.get_shared_config('LOG_LEVEL', 'INFO'), config.get_shared_config('LOG_FILE', None))
...
bot = commands.Bot(command_prefix=config.get_discord_config('COMMAND_PREFIX', '?'), intents=intents)
...
BotLogger.system_event("機器人初始化", f"環境: {ENV}, 前綴: {config.get_discord_config('COMMAND_PREFIX', '?')}")
