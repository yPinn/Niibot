...
class Reply(commands.Cog):
    ...
    async def on_message(self, message: discord.Message):
        ...
        msg = [
            "不要 @ 我，幹你娘！！！", "不熟N標", "?",
            "幹你娘機掰標三小", "皮 ↘ 炎 ↗", f"不用 標<@{target_user_id}>就好", "uu：愛是寂寞人"
        ]
    ...
    @commands.command(name="cc", aliases=["複製"], help="複製人，顯示頭像和橫幅")
            ...
            if not user:
                try:
                    ...
                    await ctx.send(embed=discord.Embed(
                        description="⚠️ 請提供有效的用戶 ID、名稱或 @提及", color=discord.Color.red()))
                    return
