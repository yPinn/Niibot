...
class EmojiTool(commands.Cog):
    ...
    async def _load_emojis(self):
        if not os.path.exists(EMOJI_JSON_PATH):
            ...
            await util.write_json(EMOJI_JSON_PATH, {"guild_emojis": {}})
        data = await util.read_json(EMOJI_JSON_PATH)
        self.emoji_data = data.get("guild_emojis", {})

    async def _load_keyword_replies(self):
        if not os.path.exists(KEYWORD_REPLY_JSON_PATH):
            await util.write_json(KEYWORD_REPLY_JSON_PATH, {})
        self.keyword_reply_map = await util.read_json(KEYWORD_REPLY_JSON_PATH)

    async def _load_emoji_stats(self):
        if not os.path.exists(EMOJI_STATS_JSON_PATH):
            await util.write_json(EMOJI_STATS_JSON_PATH, {})
        self.emoji_usage_stats = await util.read_json(EMOJI_STATS_JSON_PATH)

    async def save_emoji_stats(self):
        await util.write_json(EMOJI_STATS_JSON_PATH, self.emoji_usage_stats)
    ...
    async def export_emojis(self):
        ...
        try:
            await util.write_json(EMOJI_JSON_PATH, {"guild_emojis": result})
            await self._load_emojis()
            return True, None
    ...
    async def export_emojis_command(self, ctx):
        ...
        if success:
            await ctx.send(util.format_success_msg("已成功匯出 emoji 資訊到 JSON 並重新載入。"))
        else:
            await ctx.send(util.format_error_msg(f"匯出失敗：{err}"))
    ...
    async def emoji_stats_command(self, ctx):
        ...
        if not guild_stats:
            await ctx.send(util.format_error_msg("尚無任何 emoji 使用紀錄。"))
            return
        ...
        for name, count in sorted_stats[:10]:
            ...
            emoji_display = str(emoji) if emoji else f":{name}:"
    ...
    async def rename_emoji(self, ctx, old_name: str, new_name: str):
        ...
        if emoji:
            try:
                ...
                await ctx.send(util.format_success_msg(f"{old_name} 已重新命名為 {new_name}"))
            except discord.Forbidden:
                await ctx.send(util.format_error_msg("缺少修改 emoji 權限。"))
            except Exception as e:
                await ctx.send(util.format_error_msg(f"修改失敗：{e}"))
        else:
            await ctx.send(util.format_error_msg(f"找不到 emoji: {old_name}"))
    ...
    async def delete_emoji(self, ctx, name: str):
        ...
        if emoji:
            try:
                await emoji.delete()
                await ctx.send(util.format_success_msg(f"已刪除 emoji: {name}"))
            except discord.Forbidden:
                await ctx.send(util.format_error_msg("缺少刪除 emoji 權限。"))
            except Exception as e:
                await ctx.send(util.format_error_msg(f"刪除失敗：{e}"))
        else:
            await ctx.send(util.format_error_msg(f"找不到 emoji: {name}"))
